...
export function NavbarClient({ user, avatarUrl }: { user: User | null; avatarUrl?: string | null }) {
  ...
  const NavLinks = ({ onNavigate }: { onNavigate?: () => void }) => (
    <>
      ...
      <Link href="/evidence" className="flex cursor-pointer select-none items-center rounded-sm px-3 py-3 text-base min-h-[44px] outline-none transition-colors hover:bg-accent hover:text-accent-foreground md:min-h-0 md:py-0 md:inline-flex md:px-0 md:text-sm md:font-medium md:hover:bg-transparent md:hover:text-primary" onClick={onNavigate}>בנק ראיות</Link>
